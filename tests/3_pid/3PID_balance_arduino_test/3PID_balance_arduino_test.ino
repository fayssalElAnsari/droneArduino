...
float Gyro_angle[2];
float Total_angle[2];
float Previous_total_angle[2];
float Total_angle_first_derivative[2];
float Previous_total_angle_first_derivative[2];
float Total_angle_second_derivative[2];
...
float pid_fb_d = 0;
//yaw variables
float PIDyaw, ERROR_yaw, previous_ERROR_yaw;
float pid_yaw_p = 0;
float pid_yaw_i = 0;
float pid_yaw_d = 0;
...
/////////////////PIDyaw CONSTANTS/////////////////
double kp_yaw = 3.55; //3.55
double ki_yaw = 0.005; //0.003
double kd_yaw = 2.05; //2.05
///////////////////////////////////////////////
...
void loop() {
  ...
  */
  ...
  /*---Z---*/
  //need to test the accel angle [2]
  Acceleration_angle[2] = atan((sqrt(pow((Acc_rawY / 16384.0), 2) + pow((Acc_rawY / 16384.0), 2)) / (Acc_rawZ / 16384.0))) * rad_to_deg;
  ...
  ERROR_yaw = Total_angle[2] - desired_angle_yaw;
  ...
  PIDyaw = pid_yaw_p + pid_yaw_i + pid_yaw_d;
  ...
  //need to try the yaw fix to the pwmMotors
  
  pwmLeft = throttle + PIDlr - PIDyaw;
  pwmRight = throttle - PIDlr - PIDyaw;
  //fb
  pwmForward = throttle + PIDfb + PIDyaw;
  pwmForward = throttle - PIDfb + PIDyaw;
  ...
  Previous_total_angle[1] = Total_angle[1];
}//end of loop void
